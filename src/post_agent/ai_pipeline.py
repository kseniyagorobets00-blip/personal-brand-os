...
def _user_prompt(context: dict[str, Any]) -> str:
    return (
        ...
        "Жесткое правило языка: язык определяется только target_publication.platform. LinkedIn — все поля и draft только на английском. "
        "Telegram, VC и Сетка — все поля и draft только на русском. Нельзя дословно переводить заголовок тренда; извлеки смысл и сформулируй новую редакционную тему под площадку. "
        "draft должен быть готовым постом на языке выбранной площадки, написанным через Author Brain. "
